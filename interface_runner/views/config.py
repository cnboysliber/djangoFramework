...
class ConfigView(GenericViewSet):
    ...
    def all(self, request, **kwargs):
        ...
        try:
            env_id = request.query_params["env_id"]
            queryset = self.get_queryset().filter(project__id=pk, env_id=env_id). \
    ...
    def update(self, request, **kwargs):
        ...
        case_step = models.CaseSuitInfo.objects.filter(method="config", name=config.name)
    ...
    def delete(self, request, **kwargs):
        ...
        try:
            if kwargs.get('pk'):  # 单个删除
                ...
                if models.CaseSuitInfo.objects.filter(method="config", name=config_obj.name).exists():
...
class VariablesView(GenericViewSet):
    ...
    def add(self, request):
        ...
        if ser.is_valid():
            ...
            request.data["project"] = project  # project是一个对象，使用了外键，先实例化外键查询，然后再插入的表里面放入实例化后的外键连接
