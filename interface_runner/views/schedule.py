...
class ScheduleView(GenericViewSet):
    ...
    定时任务增删改查
    """
    queryset = celery_models.PeriodicTask.objects
    ...
    def delete(self, request, **kwargs):
        """删除任务
        """
        task = celery_models.PeriodicTask.objects.get(id=kwargs["pk"])
        task.enabled = False
        task.delete()
    ...
    def run(self, request, **kwargs):
        task = celery_models.PeriodicTask.objects.get(id=kwargs["pk"])
        task_name = 'interface_runner.tasks.schedule_debug_suite'
        args = eval(task.args)
