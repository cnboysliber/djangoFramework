...
class BmpManagerApi():
    ...
    # 密码登录接口
    def bmp_mgr_login_api(self, username=None, password=None):
        url = self.url_conf.get_bmp_api_url("MGR_LOGIN", "login")
        ...
        if password != None:
            ...
            pub_key = self.url_conf.get_bmp_api_url("PUBKEY", "mgr_pubkey")
            key_load = rsa.PublicKey.load_pkcs1_openssl_pem(pub_key)
            pwd_rsa = rsa.encrypt(password.encode('utf-8'),key_load)
            pwd_base64 = base64.b64encode(pwd_rsa).decode()
            data["password"] = str(pwd_base64)
...
if __name__ == '__main__':
    ...
    # print(out)
    status_code, text = res.bmp_mgr_login_api(username=usr,password=pwd)
