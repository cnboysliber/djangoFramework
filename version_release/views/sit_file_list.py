...
class fileListView(GenericViewSet):
    """
    版本提测增删改查
    """
    queryset = models.sitFileList.objects.all()
    serializer_class = serializers.SitFileListSerializer
    ...
    def modify(self, request, **kwargs):
        ...
        scriptList = json_data['scriptList']
        i=0
        for i in range(len(scriptList)):
            scriptList[i]['project_id']=int(scriptList[i]['project_id'])
            if scriptList[i]['project']:
                del scriptList[i]['project']
                del scriptList[i]['id']
        df = pd.DataFrame(scriptList)
        result = df
        ...
        engine = create_engine(
            ...
            encoding='utf-8')
        con = engine.connect()
        result.to_sql(name='version_release_sitfilelist', con=con, if_exists='append', index=False)
